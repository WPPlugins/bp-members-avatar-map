...
//***************************************************************************************************
function showAddress(lik,det,cords,info) {
if(cords=="")return;
...
function showAddress_online(lik,det,cords,info) {
if(cords=="")return;
var avat=document.getElementById(det).getElementsByTagName("img")[0].src;
var arr = cords.split(",");
if (arr.length!=2)return;
...
<?php if ( bp_has_members('type=active&max='.$map_max_members.'&per_page='.$map_max_members.'') ) : ?>
 ...
 try{jQuery(document).ready( function() {showAddress("<?php str_replace('"',"'",bp_member_permalink()); ?>","<?php str_replace('"',"'",bp_member_name()); ?>","<?php str_replace('"',"'",bp_member_profile_data('field=Latitude and Longitude')); ?>","<?php str_replace('"',"'",bp_member_profile_data('field=Info')); ?>");} );}
catch(err){}
<?php endwhile; ?>
<?php endif; ?>
<?php if ( bp_has_members( 'type=online' ) ) : ?>
					<?php while ( bp_members() ) : bp_the_member(); ?>   
try{jQuery(document).ready( function() {showAddress_online("<?php str_replace('"',"'",bp_member_permalink()); ?>","<?php str_replace('"',"'",bp_member_name()); ?>","<?php str_replace('"',"'",bp_member_profile_data('field=Latitude and Longitude')); ?>","<?php str_replace('"',"'",bp_member_profile_data('field=Info')); ?>");} );}
catch(err){}
<?php endwhile; ?>
<?php endif; ?>
}) 
  ...
  <?php if ( bp_has_members('type=active&max='.$map_max_members.'&per_page='.$map_max_members.'') ) : ?>
					<?php while ( bp_members() ) : bp_the_member();?>			
 <div style="display:none;" id="<?php bp_member_name() ?>"><?php bp_member_avatar() ?></div>
<?php endwhile; ?>
<?php endif; ?>
